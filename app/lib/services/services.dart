...
class AuthService {
  ...
  Future<void> logout() async {
    try {
      ...
      // Swallowed, and that is the whole point of the paragraph above. The
      // refusal is already in the diagnostic log with the server's own words
      // on it; rethrowing it here would hand the crew an error off a button
      // whose work is done, and on a shared line the refusal is usually the
      // rate limiter rather than anything about this sign-out.
    } finally {
...
/// The one thing this app reads about the plant's accounts.
///
/// A port of the shop floor's half of client/src/api/services/user.service.ts.
/// Everything else there - listing accounts, creating one, renaming one,
/// resetting a PIN - is the back office's and is refused this app's token, so
/// none of it is ported.
class UserService {
  UserService(this._api);
  final ApiClient _api;

  /// The names the plant's own accounts say may sign a record.
  ///
  /// Names, not accounts: the tablet needs to know who may sign, not who exists
  /// or what role they hold. Feeds the Supervisor pick, which used to read a
  /// list of three names compiled into this app - so a supervisor renamed or
  /// added in the back office never reached the floor at all.
  Future<List<String>> signers() => _api.get(
    Endpoints.userSigners,
    parse: (d) => d is List
        ? [
            for (final name in d)
...
class BatchService {
  ...
  /// Every batch number on record - closed as well as open, and the coarse and
  /// DRC charges alongside the special ones.
  ///
  /// Only the Batches tab's All filter asks for this, and only when somebody
  /// taps it. listOpen() above is what the floor works from and what the refiner
  /// pickers read: a closed batch must not be offerable, and this list is full
  /// of them.
  ///
  /// Paged through rather than asked for in one go, because the API clamps any
  /// limit to 200 (see parsePagination) and says nothing about having done it.
  /// The plant is at 175 batches, so a single call works today and would start
  /// quietly dropping the oldest ones the month it passed two hundred - on a
  /// screen whose whole purpose is that every number is on it. The page count
  /// comes from the server's own meta, and the loop is bounded by it rather
  /// than by trusting a short page to mean the end.
  Future<List<Batch>> listAll() async {
    const perPage = 200;
    final all = <Batch>[];
    var page = 1;
    var pages = 1;
    while (page <= pages) {
      ...
      all.addAll(res.rows);
      pages = res.meta.pages > 0 ? res.meta.pages : 1;
