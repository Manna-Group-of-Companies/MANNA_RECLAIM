    ...
    await showAppSheet<void>(
      ...
      body: (context, setSheetState) => Column(
        ...
        children: [
          ...
          // A machine can have four bearings, and four boxes down a phone-width
          // sheet is a scroll in the middle of taking readings with a probe in
          // one hand. Two across wherever the sheet is wide enough for it.
          FieldColumns(
            children: [
              ...
              builder: (_) {
                ...
                return Column(
                  ...
                  children: [
                    ...
                    // A line's machines go across the screen rather than down
                    // it once there is room: a supervisor walking in wants the
                    // whole autoclave bank in one look, and on a tablet that is
                    // two or three cards a row.
                    CardGrid(
                      children: [
                        for (final machine in entry.value)
                          MachineCard(
                            ...
                            // A machine that can be on either line asks which
                            // one first; the answer opens the start sheet.
                            onStart: (m) =>
                                isDualLine(m) ? _askLine(m) : _openStart(m),
                            onStop: _openStop,
                            onPause: (r, paused) async {
                              final ui = context.read<UiStore>();
                              final okay = await context
                                  .read<RunsStore>()
                                  .pause(r.id, paused);
                              if (okay) {
                                ui.notify(
                                  paused ? 'Run paused' : 'Run resumed',
                                );
                              }
                            },
                            onTally: _openTally,
